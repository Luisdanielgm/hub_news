    ...
    try:
        ...
        for article in articles:
            ...
            if not article_exists(collection, link_url):
                print('-- Nueva noticia en Xataka agregada:')
                print(article_info)
                print('-------------------------------------')
                ...
                for element in texts_full.children:
                    if element.name == 'div':
                        ...
                        exclusion_strings = ['imágenes', 'imágenes |', 'imágenes: ', 'imagen', 'imagen: ', 'imagen |', 'más información: ', 'más información', 'más información |', 'en xataka:', 'en xataka |']

                        if any(text_lower.startswith(s) for s in exclusion_strings):
                            continue  # Omitir la adición de este texto
                        else:
                            # Agregar el texto al extracted_text con saltos de línea adicionales
                            extracted_text += text + '\n\n'
        ...
        print(f"Se han extraido {len(scraped_data)} noticias de Xataka.")
        return scraped_data

    except Exception as e:
        logging.error(f"Error durante el scraping de Xataka: {e}")
        return []

# Función para ejecutar el scraping de forma asíncrona
def run_scraping():
    loop = asyncio.get_event_loop()
    return loop.run_until_complete(scrape_xataka())